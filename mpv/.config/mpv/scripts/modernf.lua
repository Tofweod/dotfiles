...
opt.read_options(user_opts, "modernf", function(list)
	update_options(list)
end)
-- apply lang opts
	...
	for n = 1, #elements do
		...
		if element.type == "slider" then
			...
			if element.slider.seekRangesF ~= nil then
				seekRanges = element.slider.seekRangesF()
			end
			if seekRanges then
				elem_ass:draw_stop()
...
layouts["reduced"] = function()
	...
	lo = add_layout("title")
	lo.geometry = geo
	lo.style = string.format("%s{\\clip(%f,%f,%f,%f)}", osc_styles.Title, geo.x, geo.y - geo.h, geo.x + geo.w, geo.y)
	lo.alpha[3] = 0
...
function osc_init()
	...
	ne.eventresponder["mbtn_right_up"] = function()
		mp.commandv("script-binding", "playlistmanager/showplaylist")
	end
	...
	ne.eventresponder["mbtn_left_down"] =
		...
		function()
			mp.commandv("seek", -90, "relative", "keyframes")
		end
	...
	ne.eventresponder["mbtn_left_down"] =
		...
		function()
			mp.commandv("seek", 87, "relative", "keyframes")
		end
	...
	ne.eventresponder["mbtn_right_up"] = function()
		mp.commandv("script-binding", "stats/display-page-4")
	end
	-- title
	...
	ne.eventresponder["wheel_up_press"] = function(element)
		set_volume(state.proc_volume + 5)
	end
	ne.eventresponder["wheel_down_press"] = function(element)
		set_volume(state.proc_volume - 5)
	end
	-- tc_left (current pos)
...
function process_event(source, what)
	...
	elseif source == "mouse_move" then
		...
		if
			...
			or (
				not ((state.last_mouseX == nil) or (state.last_mouseY == nil))
				and (
...
mp.set_key_bindings({
	...
	},
	{
		"mbtn_mid",
		function(e)
			process_event("mbtn_mid", "up")
		end,
		function(e)
			process_event("mbtn_mid", "down")
		end,
	},
	{
		"wheel_up",
...
function idlescreen_visibility(mode, no_osd)
	...
	-- utils.user_data("osc-idlescreen", mode)
