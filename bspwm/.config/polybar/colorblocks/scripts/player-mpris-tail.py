...
FORMAT_REGEX = re.compile(
    r'(\{:(?P<tag>.*?)(:(?P<format>[wt])(?P<formatlen>\d+))?:(?P<text>.*?):\})',
    re.I,
)
...
class PlayerManager:
    ...
    def onChangedProperties(
        self, interface, properties, signature, sender=None
    ):
    ...
    def getBusNameFromOwner(self, owner):
        player_bus_names = [
            bus_name
            for bus_name in self._session_bus.list_names()
            if self.busNameIsAPlayer(bus_name)
        ]
    ...
    def getSortedPlayerOwnerList(self):
        ...
        return [
            ...
            for info in reversed(
                sorted(players, key=itemgetter('status', 'number'))
            )
        ]
    ...
    def getCurrentPlayer(self):
        playing_players = [
            ...
            if self.players[player_owner].status == 'playing'
            or self.players[player_owner].status == 'paused'
        ]
...
class Player:
    def __init__(
        self, session_bus, bus_name, owner=None, connect=True, _print=None
    ):
        ...
        self.metadata = {'artist': '', 'album': '', 'title': '', 'track': 0}

        self._rate = 1.0
        self._positionAtLastUpdate = 0.0
    ...
    def connect(self):
        if self._disconnecting is not True:
            ...
            if 'TrackMetadataChanged' in introspect_xml:
                self._signals['track_metadata_changed'] = (
                    self._session_bus.add_signal_receiver(
            ...
            self._signals['properties_changed'] = (
    ...
    def updateIcon(self):
        ...
        self.icon_reversed = (
            ICON_PAUSED if self.status == 'playing' else ICON_PLAYING
        )
    ...
    def _parseMetadata(self):
        if self._metadata != None:
            ...
            _length = _getProperty(
                self._metadata, 'xesam:length', 0
            ) or _getProperty(self._metadata, 'mpris:length', 0)
            _length_int = (
                _length if type(_length) is int else int(float(_length))
            )
            _fmt_length = (  # Formats using h:mm:ss if length > 1 hour, else m:ss
                ...
                else f'{_length_int/1e6//3600:.0f}:{_length_int/1e6%3600//60:02.0f}:{_length_int/1e6%60:02.0f}'
            )
            ...
            _cover = _getProperty(
                self._metadata, 'xesam:artUrl', ''
            ) or _getProperty(self._metadata, 'mpris:artUrl', '')
    ...
    def onPropertiesChanged(self, interface, properties, signature):
        ...
        if dbus.String('PlaybackStatus') in properties:
            ...
            if status != self.status:
                ...
                updated = True
        if (
            dbus.String('Rate') in properties
            and dbus.String('PlaybackStatus') not in properties
        ):
            self.refreshStatus()
    ...
    def checkPositionTimer(self):
        if (
            ...
            and not self._positionTimerRunning
        ):
    ...
    def _getPosition(self):
        if self.status == 'playing':
            return self._positionAtLastUpdate + self._rate * (
                time.time() - self._timeAtLastUpdate
            )
        else:
        ...
        if format is not None:
            ...
            if format == 'w':
                ...
                if len(text) > formatlen:
                    text = (
                        text[: max(formatlen - len(TRUNCATE_STRING), 0)]
                        + TRUNCATE_STRING
                    )
    ...
    def printStatus(self):
        if self.status in ['playing', 'paused']:
            ...
            text = re.sub(
                FORMAT_REGEX,
                lambda match: self._statusReplace(match, metadata),
                FORMAT_STRING,
            )
            ...
            try:
                text = re.sub(
                    r'􏿿p􏿿(.*?)􏿿p􏿿(.*?)􏿿p􏿿(.*?)􏿿p􏿿',
                    r'%{\1}\2%{\3}',
                    text.format_map(CleanSafeDict(**metadata)),
                )
            except:
...
def _dbusValueToPython(value):
    if isinstance(value, dbus.Dictionary):
        return {
            _dbusValueToPython(key): _dbusValueToPython(value)
...
parser.add_argument(
    'command',
    help="send the given command to the active player",
    choices=[
        'play',
        'pause',
        'play-pause',
...
parser.add_argument(
    '-b',
    '--blacklist',
    help="ignore a player by it's bus name. Can be given multiple times (e.g. -b vlc -b audacious)",
    action='append',
    metavar="BUS_NAME",
    default=[],
)
parser.add_argument(
    '-w',
    '--whitelist',
    help="permit a player by it's bus name like --blacklist. will block --blacklist if given",
    action='append',
    metavar="BUS_NAME",
    default=[],
)
parser.add_argument(
    '-f',
    '--format',
    default='{icon} {:artist:{artist} - :}{:title:{title}:}{:-title:{filename}:}',
)
parser.add_argument('--truncate-text', default='…')
parser.add_argument('--icon-playing', default='󰐊')
parser.add_argument('--icon-paused', default='󰏤')
parser.add_argument('--icon-stopped', default='󰓛')
parser.add_argument('--icon-none', default='')
args = parser.parse_args()

FORMAT_STRING = re.sub(
    r'%\{(.*?)\}(.*?)%\{(.*?)\}', r'􏿿p􏿿\1􏿿p􏿿\2􏿿p􏿿\3􏿿p􏿿', args.format
)
...
if args.command is None:
    ...
    player_manager = PlayerManager(
        filter_list=filter_list, block_mode=block_mode, connect=False
    )
    ...
    if args.command == 'play' and current_player:
        ...
        print(
            "\n".join(
                sorted(
                    [
                        "{} : {}".format(
                            player.bus_name.split('.')[3], player.status
                        )
        ...
        print(
            "{} : {}".format(
                current_player.bus_name.split('.')[3], current_player.status
            )
        )
